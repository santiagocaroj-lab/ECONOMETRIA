...
if uploaded_data is not None:
    ...
    # MEJORA 2: Definición de una lista explícita de stubnames
    # Esto dota de máxima robustez a la función wide_to_long frente a múltiples guiones bajos
    # e incluye variantes de nombres comúnmente encontradas (RANK y RANKING).
    ...
    # Despliegue modular a través de pestañas funcionales sin alterar los componentes existentes
    ...
    # =========================================================================
    with tab2:
        st.subheader("1. Evolución Promedio Regional")
        columnas_promedio = [col for col in ['RSF_SCORE', 'CORRUPCION', 'IED_PIB'] if col in df_long.columns]
        df_promedio = df_long.groupby('Año')[columnas_promedio].mean().reset_index()
        
        fig_line = px.line(df_promedio, x='Año', y=columnas_promedio, 
                           title='Evolución Promedio en la Región', markers=True)
        st.plotly_chart(fig_line, use_container_width=True)

        st.subheader("2. Gráficos Individuales por País")
        pais_seleccionado = st.selectbox("Seleccione un país para examinar", df_long['País'].unique())
        df_pais = df_long[df_long['País'] == pais_seleccionado]
        fig_pais = px.line(df_pais, x='Año', y=columnas_promedio, 
                           title=f'Trayectoria Temporal en {pais_seleccionado}', markers=True)
        st.plotly_chart(fig_pais, use_container_width=True)

        st.subheader("3. Diagramas de Dispersión (Scatter Plots) con Línea de Tendencia")
    ...
    with tab3:
        ...
        # MEJORA 3: Función enriquecida de interpretación automática orientada a la sustancia jurídico-económica
        ...
        if not df_m1.empty:
            ...
            # MEJORA 1: Adición simultánea de efectos fijos individuales y temporales (entity_effects y time_effects)
        ...
        if not df_m2.empty:
            ...
            # MEJORA 1: Control estricto por choques comunes anuales y perfiles estructurales de los países
        ...
        if 'IED_PIB' in df_panel.columns and 'IED_L1' in df_panel.columns:
            ...
            # MEJORA 4: El uso selectivo de dropna() garantiza mantener observaciones parciales,
            # aislando exclusivamente celdas vacías (como el año 2025 o Cuba) sin comprometer el resto del panel.
            df_m3 = df_panel[vars_mod3].dropna()
            if not df_m3.empty:
                X3 = sm.add_constant(df_m3[['CORRUPCION_L1', 'IED_L1']])
                Y3 = df_m3['IED_PIB']
                
                # MEJORA 1: Estimación robusta incorporando dobles efectos fijos
        ...
        if 'IED_PIB' in df_panel.columns and 'IED_L1' in df_panel.columns:
            ...
            # MEJORA 4: Tratamiento conservador de faltantes por fila de análisis para los modelos de IED
            df_m4 = df_panel[vars_mod4].dropna()
            if not df_m4.empty:
                X4 = sm.add_constant(df_m4[['RSF_L1', 'IED_L1']])
                Y4 = df_m4['IED_PIB']
                
                # MEJORA 1: Modelación bidimensional con efectos cruzados estables
